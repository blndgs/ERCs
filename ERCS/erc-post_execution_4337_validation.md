...
Implementers can design their validation logic to execute conditionally based on the specific requirements of the user operation. Similar to the [`executeUserOp`](../assets/erc-post_execution_4337_validation/entrypoint_0.7.pdf) 4-byte selector dispatch mechanism, the `validatePostExecution` selector within the userop signature signals the EntryPoint to call it.
...
The `EntryPoint` contract's `handleOps` function is modified to call `_validatePostExecution` for each UserOperation, which executes account post-execution validation. The validation dispatch follows the EntryPoint v0.7.0 pattern of calling conditionally the `execUserOperation` function, based on the detection of the `validatePostExecution` selector in the userOp's signature.
